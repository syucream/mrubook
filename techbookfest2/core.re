...
いわゆるインタプリタ形式の言語処理系においてコンパイル処理は字句解析、構文解析、コード生成、最適化からなると思われます。
ちなみに字句解析はソースコードをトークン列に分割する処理、構文解析はトークン列を読んで構文に基づいて行なう処理（後続の処理のため構文木を生成したりする）、コード生成は中間表現の生成処理、最適化は実行時の無駄を少なくする処理です。

mruby でもこれらに則して、まずスクリプトは中間表現にコンパイルした後、中間表現をターゲットとして実行します。
この中間表現を解釈して実行するのが mruby の Virtual Machine(VM) である RiteVM です。
またこの中間表現をコンパイルするのが、字句・構文解析器とコード生成器からなるコンパイラ部分です。
なお mruby ではコンパイラの字句解析のうちキーワード検出については専用のハッシュ関数を用います。
...
body について補足すると、これは前述の irep の他に関数ポインタを指定することもできます。
これは mrbgem を C で書くような人が、 RProc 型オブジェクトを生成して何か処理をさせたい時に有用でしょう。
この関数ポインタの扱いのため、後述する RiteVM では RProc の body が関数ポインタの場合にそれを呼び出すような条件分岐を行っています。
本書では詳しくは触れないのですが、もし興味がありましたら src/vm.c 中の MRB_PROC_CFUNC_P マクロを使って分岐しているあたりを読んでみてください。
...
命令	内容
-------------------------------------------------------------
OP_GETGLOBAL A Bx	R(A) に Syms(Bx) が指すグローバル変数の値をコピーする
OP_SETGLOBAL A Bx	R(A) の値を Syms(Bx) が指すグローバル変数の値として格納する
...
OP_JMPIF A sBx	R(A) の評価結果が true だった場合、 sBx 分だけプログラムカウンタを増やす
...
//image[stack][stack 領域の使われ方][scale=0.9]
...
この処理は mrb_context_run() 中の OP_ONERR や OP_EPUSH の処理の case 文の内部で行われています。

なおこのような倍々に領域を増やすというロジックは mruby の実装では例外処理周りのみならずよく見られるパターンです。
具体的には mrb->c->stack を伸長する stack_extend() や、 mrb->c->ci に要素を push する際に伸長する可能性がある cipush() でも同様だったりします。
