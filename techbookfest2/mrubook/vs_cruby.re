...
ここでは主に、筆者が気になった 2 点、 mruby で使用されるハッシュ関数ライブラリ khash と ガベージコレクションについて簡単に触れてみます。
...
すなわちルートオブジェクトから、オブジェクトの参照関係を辿れるだけ辿って到達できたオブジェクトを生きているオブジェクトとしてマークし、マークがついていないオブジェクトは死んでいるとみなして回収することで GC を実現します。

ただし単純にマークスイープするのではなく、 mruby の利用想定シーンである組み込み機器への適用を見越して、一回の GC サイクルで全てを行わずに段階的にサイクルを実行するインクリメンタル GC 採用しています。
...
現実に起こりうる重大な問題としては「回収されてはいけないオブジェクトが、マークフェーズ中に関連づいたためマークされず回収されてしまった」シナリオが考えられます。
...
//image[gc_flow][GC の実行フロー]

mrb_incremental_gc() はフェーズ毎の具体的な GC 処理を 2 個の関数、メジャー GC 実行時は incremental_gc_step() 、マイナー GC 実行時は incremental_gc_until() を呼び出すことで実行します。
...
前節で出てきた incremental_gc_step() や incremental_gc_until() では具体的な GC 処理を incremental_gc() を呼び出すことで進行します。
...
これによって既にマークされたオブジェクトをマークフェーズの対象としなくする、世代別 GC を実現しています。
