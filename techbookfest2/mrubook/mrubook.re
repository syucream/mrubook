 ...
 ** nregs にレジスタ数が格納される
 ** 16 ビットに(ry
 * irep の解釈の仕方を制御するためのフラグ
 * iseq
 ** コードセグメントの先頭番地へのポインタ
 ** 実体としては 32 ビット整数値(mrb_code) のポインタ
 ...
 ** 続きの？ mrb_irep のリスト
 * ローカル変数配列
 ** mrb_locals という構造体で管理
 ** 実体としてはシンボルと符号なし整数値のペア
 * 各種カウンタ
 ** TODO
...
まず mruby の字句解析ですが、パーサは parse.y 、レキシカルアナライザーは lex.def にあります。
lex.def ですが、これはゼロから実装されている訳ではなく、keywords に定義されている mruby のキーワードを検出するために GNU Perfect(gperf) で生成した完全ハッシュ関数を利用しています。
...
mruby の一番の特徴は、 mruby の VM (RiteVM とも呼ぶらしい) にあるとも考えられるでしょう。
mruby の VM についてつらつらと書いていきます。

=== アーキテクチャ

VM の実装を読み解く上で命令セットアーキテクチャの理解は不可欠です。
というわけで、まずは mruby の命令について理解を深めてみましょう。
なお、ソースコード上は include/mruby/opcode.h がこの内容に該当します。

mruby の VM はレジスタマシンとして動作します。
1 つの命令は 32 ビットで表現され、オペコードと 0 個以上 3 個以上のオペランドを取る形式になります。
0 個以上 3 個以上のオペランド・・・と言いましたが、これは命令のオペランド部の使い方によって 4 タイプの派生を持っています。

 * ここに図を描く

mruby のオペコードについては全部列挙していくとキリがないため一部だけ抜粋して紹介することにします。
 ...
 * ロード系
 ** OP_MOVE A B
 *** A が指すレジスタに B が指すレジスタの値をコピーします
 ** OP_LOADL A Bx
 *** A が指すレジスタに B が指す pool? の値をコピーします
 * 変数操作系
 ** OP_GETGLOBAL A Bx
 *** A が指すレジスタに B が指すシンボルのグローバル変数の値をコピーします
 * 制御系
 ** OP_JMP sBx
 ** OP_JMPIF A sBx
 ** OP_SEND A B C
...
mrb_state は VM の実行状態情報を保持するデータ構造です。
おそらく C で mrbgem を実装しようとする人はちらほら目にすることになるでしょう。
