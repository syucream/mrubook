...
具体的には mruby のコンパイラや Virtual Machine（VM） 、コア機能（本書ではハッシュテーブルと GC にフォーカス）について触れていきます。
...
具体的には Ruby の JIS/ISO 規格@<bib>{iso_draft} @<bib>{spec} に記載がある機能は標準で搭載されているのですが、それ以外はオプションな立ち位置で組み込むか選択可能だったり、そもそも実装されていなかったりします。
...
残念ながら mruby-1.2.0 は本書執筆時の一年以上前にリリースされたものですので、本書で解説する内容は最新のソースコードの内容と齟齬が発生している可能性があります。
